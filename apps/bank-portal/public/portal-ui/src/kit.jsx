...
function resolveRouteFromLocation() {
  ...
  if (hashRoute) {
    const cleanPath = cleanPathForRoute(hashRoute);
    if (cleanPath) {
      ...
      if (currentPath !== cleanPath || window.location.hash) {
        ...
        url.hash = "";
        window.history.replaceState({ route: hashRoute }, "", url);
      }
    }
    return hashRoute;
  }
...
function cleanPathForRoute(route) {
  return CLEAN_ROUTE_PATHS[normalizeRoute(route)] || null;
}

function appHostsBankSurface() {
  const bankHomePath = cleanPathForRoute("/bank/home");
  return bankHomePath != null && (bankHomePath === "/app/dashboard" || bankHomePath.startsWith("/app/"));
}

function originForSurface(surface) {
  const configured = window.__PORTAL_ORIGINS && window.__PORTAL_ORIGINS[surface];
  if (configured) {
    try {
      return new URL(configured, window.location.origin).origin;
    } catch {
      // Ignore malformed configuration and continue with fallback.
    }
  }

  const url = new URL(window.location.href);
  if (surface === "bank" && url.port === "3000") {
    url.port = "3001";
  } else if (surface === "company" && url.port === "3001") {
    url.port = "3000";
  }
  return url.origin;
}

function targetOriginForRoute(route) {
  const normalized = normalizeRoute(route) || "/login";
  const isBankRoute = normalized.startsWith("/bank/");
  const isCompanyRoute = normalized.startsWith("/smb/") || normalized === "/search";

  if (!isBankRoute && !isCompanyRoute) {
    return window.location.origin;
  }

  const currentHostsBank = appHostsBankSurface();
  const targetSurface = isBankRoute ? "bank" : "company";
  const currentSurface = currentHostsBank ? "bank" : "company";

  if (targetSurface === currentSurface) {
    return window.location.origin;
  }

  return originForSurface(targetSurface);
}
...
function hrefForRoute(origin, route) {
  ...
  if (!cleanPath) {
    return `${origin}/portal-ui/index.html#${normalized}`;
  }
  return `${origin}${cleanPath}`;
}

function emitRouteChange() {
  window.dispatchEvent(new Event("portal-ui-route-change"));
}

function navigateToRoute(route, options = {}) {
  const normalized = normalizeRoute(route) || "/login";
  const targetOrigin = targetOriginForRoute(normalized);

  if (targetOrigin !== window.location.origin) {
    const href = hrefForRoute(targetOrigin, normalized);
    if (options.replace) {
...
function AskCopilotFAB({ go, lang = "en", target = "/smb/copilot", hidden = false }) {
  ...
  const navigate = (p) => {
    ...
    if (window.PortalUIRouter && typeof window.PortalUIRouter.navigateToRoute === "function") {
      window.PortalUIRouter.navigateToRoute(p);
      return;
    }
...
Object.assign(window, {
  ...
  PortalUIRouter: {
    appHostsBankSurface,
    originForSurface,
    targetOriginForRoute,
    cleanPathForRoute,
    hrefForRoute,
    navigateToRoute,
    resolveRouteFromLocation,
  },
  // Backward-compatible alias for legacy references during migration.
  PrototypeRouter: {
    cleanPathForRoute,
    hrefForRoute,
    navigateToRoute,
