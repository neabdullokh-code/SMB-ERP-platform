        ...
        <table className="tbl">
          ...
          <tbody>{BOMs.map(b =>
            <tr key={b.id}>
              ...
              <td className="row-actions"><Button size="sm" variant="ghost" onClick={() => go("/smb/production/order")}>Schedule</Button></td>
            </tr>)}</tbody>
        </table>
      </div>
    </Placeholder>
...
function ServicesKanban() {
  const [tasks, setTasks] = useStateS([
    { id: 1001, c: "Oriental Trade", t: "Delivery · Tashkent", a: "JA", status: "Requested" },
    { id: 1002, c: "Retail Centre", t: "Pickup", a: "BY", status: "Requested" },
    { id: 1010, c: "Zamon Foods", t: "Cold chain delivery", a: "MK", status: "Approved" },
    { id: 1020, c: "Kamolot branch #2", t: "Internal transfer", a: "BY", status: "In progress" },
    { id: 1021, c: "Nur Auto Parts", t: "Inventory audit", a: "JA", status: "In progress" },
    { id: 1030, c: "Chorsu Market", t: "Delivery", a: "BY", status: "Completed" },
    { id: 1031, c: "Ferghana Agro", t: "Pickup", a: "MK", status: "Completed" },
  ]);

  const stages = [
    { n: "Requested",   tone: "info" },
    { n: "Approved",    tone: "warn" },
    { n: "In progress", tone: "ai" },
    { n: "Completed",   tone: "good" },
  ];
        ...
        {stages.map((col, i) => (
          <div key={i} className="card card-pad-0">
            <div className="panel-title">
              <Pill tone={col.tone} dot={false}>{col.n}</Pill>
              <span className="sp"/>
              <span className="mono muted" style={{fontSize:11}}>{tasks.filter(t => t.status === col.n).length}</span>
            </div>
            <div style={{padding:8}}>
              {tasks.filter(t => t.status === col.n).map((w) => (
                <div key={w.id} className="hairline" style={{padding:10, borderRadius:6, marginBottom:8}}>
                  ...
                  <div className="row mt-8">
                    ...
                    <div className="row gap-4">
                      {i < 3 && (
                        <button className="icon-btn sm" title="Next stage" onClick={() => move(w.id, stages[i+1].n)}>
                          <Icon.ChevRight size={12}/>
                        </button>
                      )}
                    </div>
                    <span className="mono muted" style={{fontSize:10}}>WO-{w.id}</span>
                  </div>
                </div>
              ))}
            </div>
