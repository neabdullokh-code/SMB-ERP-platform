...
import { BANK_PROTECTED_PORTAL_PATHS, BANK_PUBLIC_PORTAL_PATHS, mapBankRedirectPath, mapCompanyRedirectPath, resolvePortalUrl } from "@sqb/config/portal";
...
async function tryFetchSession(url: URL, cookieHeader: string): Promise<{ role: string; requiresTermsAcceptance: boolean } | null> {
  ...
  if (!["bank_admin", "super_admin"].includes(body.session.role)) return null;
...
async function validateSession(request: NextRequest): Promise<SessionValidation> {
  ...
  if (sessionToken) {
    try {
      ...
      if (
        ...
        body.authenticated &&
        body.session &&
        ["bank_admin", "super_admin"].includes(body.session.role)
      ) {
  ...
  // Access token invalid/missing — attempt silent refresh
  if (!refreshToken) {
...
export async function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith("/smb/")) {
    const companyPath = mapCompanyRedirectPath(pathname);
    const companyUrl = resolvePortalUrl(companyPath, "company", {
...
export const config = {
  matcher: ["/", "/login", "/otp", "/forgot", "/terms", "/onboarding", "/app/:path*", "/smb/:path*", "/bank/:path*"]
};
