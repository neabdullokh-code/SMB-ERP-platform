...
const ROUTE_PERMISSIONS = [
  ...
  { match: (path) => path.startsWith("/smb/documents/goods-receipts"), permissions: ["inventory.manage"] },
  { match: (path) => path.startsWith("/smb/documents/goods-issues"), permissions: ["inventory.manage"] },
  { match: (path) => path.startsWith("/smb/documents/inventory-transfers"), permissions: ["inventory.manage"] },
  { match: (path) => path.startsWith("/smb/documents/production-orders"), permissions: ["production.manage"] },
        ...
        <div className="auth-card">
          ...
          <div className="col gap-12">
            ...
            <div className="row" style={{justifyContent:"center", gap:6, color:"var(--muted)", fontSize:12}}>
              New to SQB Business OS? <a style={{color:"var(--ink)", cursor:"pointer"}} onClick={() => isBankOrigin() ? window.location.href = window.PrototypeRouter.hrefForRoute(companyOrigin(), "/onboarding") : go("/onboarding")}>Create workspace</a>
            </div>
          </div>
        </div>
      </div>
...
function OnboardingPage({ go }) {
  ...
  useEffect(() => {
    ...
    window.location.replace(window.PrototypeRouter.hrefForRoute(companyOrigin(), "/onboarding"));
  }, []);
