...
function buildSystemPrompt(locale: string | undefined, context: unknown) {
  ...
  if (locale === "ru") {
    return [
      ...
      "- Никогда не упоминай технические UUID и системные идентификаторы (формат 00000000-0000-...). Используй только читаемые названия: имена, артикулы, номера накладных.",
      "- Markdown отображается: используй **жирный** для ключевых чисел и выводов, * для маркированных списков.",
